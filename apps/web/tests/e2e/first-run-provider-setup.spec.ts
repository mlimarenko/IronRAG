...
test('completes first-run setup with a hosted router provider', async ({ page }) => {
  ...
  await expect(page.getByRole('status', { name: 'https://router.example/api/v1' })).toBeVisible()
  ...
  const setupResponse = page.waitForResponse(
    (response) =>
      response.url().endsWith('/v1/iam/bootstrap/setup') && response.request().method() === 'POST',
  )
