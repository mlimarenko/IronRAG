...
const { useAppMock, adminApiMock, dashboardApiMock, librarySnapshotApiMock, queryApiMock } =
  vi.hoisted(() => ({
    useAppMock: vi.fn(),
    adminApiMock: {
      listTokens: vi.fn(),
...
describe('AdminPage routing', () => {
  ...
  it('renders the libraries catalog at /admin/libraries', async () => {
    ...
    expect(container.textContent).toContain('workspace-1/library-1')
    expect(container.querySelector('button[aria-label="Copy: workspace-1/library-1"]')).toBeTruthy()
  })
