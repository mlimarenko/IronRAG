...
function parseBindingState(value: string): AIBindingState {
  if (value === 'active' || value === 'invalid' || value === 'disabled') {
    return value
  }
  throw new Error(`AI response has invalid bindingState: ${value}`)
}

function parseProviderLifecycleState(
  value: ProviderCatalogEntryResponse['lifecycleState'],
): AIProvider['lifecycleState'] {
  switch (value) {
    case 'active':
    case 'deprecated':
    case 'disabled':
...
function mapProvider(raw: ProviderCatalogEntryResponse): AIProvider {
  ...
  return {
    ...
    lifecycleState: parseProviderLifecycleState(raw.lifecycleState),
    ...
    runtime: raw.runtime,
    uiHints:
      raw.uiHints && typeof raw.uiHints === 'object' && !Array.isArray(raw.uiHints)
...
function mapAccount(raw: AiAccountResponse, providers: AIProvider[]): AIAccount {
  const provider = providers.find((entry) => entry.id === raw.providerCatalogId) ?? {
    displayName: 'Unknown',
    kind: 'unknown',
  }
  return {
...
function mapBinding(raw: AiBindingResponse): AIBinding {
  ...
  return {
    ...
    ...(optionalNumber(raw.temperature) !== undefined
      ? { temperature: raw.temperature as number }
      : {}),
    ...(optionalNumber(raw.topP) !== undefined ? { topP: raw.topP as number } : {}),
    ...(optionalNumber(raw.maxOutputTokensOverride) !== undefined
      ? { maxOutputTokens: raw.maxOutputTokensOverride as number }
      : {}),
    ...(extraParams !== undefined ? { extraParams } : {}),
    state: parseBindingState(raw.bindingState),
  }
}

export function mapProviderList(
  raw: ProviderCatalogEntryResponse[] | null | undefined,
): AIProvider[] {
...
export function mapBindingList(raw: AiBindingResponse[] | null | undefined): AIBinding[] {
