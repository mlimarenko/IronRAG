...
export function PriceOverrideDialog({
  ...
  const resetPricingEditor = () => {
    setEditorOpen(false);
    setEditingRule(null);
    pricingForm.reset({ billingUnit: '', currency: 'USD', effectiveFrom: '', effectiveTo: '', unitPrice: '' });
  };
  ...
  return (
    ...
    >
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>{t('admin.pricingOverrideDialogTitle', { model: model?.modelName ?? '' })}</DialogTitle>
            ...
            {rows.length === 0 ? (
              ...
              <div className="space-y-2">
                {rows.map(rule => {
                  ...
                  return (
                    <div key={rule.id} className="flex items-center justify-between gap-3 rounded-lg border border-border/70 px-3 py-2">
        ...
        <DialogFooter>
          {editorOpen ? (
            <>
              ...
              >
                {pricingForm.formState.isSubmitting ? <><Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> {t('admin.saving')}</> : t('admin.save')}
              </Button>
            </>
          ) : (
