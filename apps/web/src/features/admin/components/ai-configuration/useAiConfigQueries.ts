...
const ADMIN_AI_BINDINGS_QUERY_KEY = ['admin', 'ai', 'bindings'] as const
const ADMIN_AI_PRICES_QUERY_KEY = ['admin', 'ai', 'prices'] as const
...
export function useAiConfigQueries({
  ...
  const models = useMemo<AIModelOption[]>(() => mapModelList(modelsQuery.data), [modelsQuery.data])
  const prices = useMemo<PricingRule[]>(
    () =>
      Array.isArray(pricesQuery.data)
        ? pricesQuery.data.map((entry) => mapPricing(entry, providers, models))
        : [],
  ...
  const invalidateAll = useCallback(async () => {
    await Promise.all([
      queryClient.invalidateQueries({ queryKey: ADMIN_AI_PROVIDERS_QUERY_KEY }),
      queryClient.invalidateQueries({ queryKey: ADMIN_AI_ACCOUNTS_QUERY_KEY }),
      queryClient.invalidateQueries({ queryKey: ADMIN_AI_BINDINGS_QUERY_KEY }),
      queryClient.invalidateQueries({ queryKey: ADMIN_AI_PRICES_QUERY_KEY }),
      queryClient.invalidateQueries({ queryKey: ADMIN_MODEL_CATALOG_QUERY_KEY }),
    ])
  }, [queryClient])
