...
type WorkspaceAccessRowProps = Readonly<{
  expandedIds: string[]
  libraryQueries: ReturnType<typeof useQueries>
  libraries: CatalogLibraryResponse[]
  selectedLibraries: Set<string>
  selectedWorkspaces: Set<string>
  t: ReturnType<typeof useTranslation>['t']
  toggleExpanded: (workspaceId: string) => void
  toggleLibrary: (libraryId: string, checked: boolean) => void
...
function WorkspaceAccessRow({
  ...
  const isLoading = libraryQueries[expandedIds.indexOf(workspace.id)]?.isLoading === true
  let libraryContent
  if (isLoading) {
    libraryContent = (
    ...
    libraryContent = libraries.map((library) => (
      <label key={library.id} className="flex cursor-pointer items-center gap-2.5 py-1">
        <Checkbox
    ...
    <div className="workbench-surface overflow-hidden">
      <div className="flex items-center gap-2.5 p-3">
        ...
        <label className="flex min-w-0 flex-1 cursor-pointer items-center gap-2.5">
          ...
          />
          <span className="truncate text-sm font-semibold">{workspace.displayName}</span>
        </label>
      </div>
      {isExpanded && (
        <>
          <Separator />
          <div className="space-y-1 bg-surface-sunken/40 py-2 pl-12 pr-3">{libraryContent}</div>
        </>
      )}
    </div>
  )
 ...
 */
export function UserAccessDialog({ user, open, onOpenChange }: Readonly<UserAccessDialogProps>) {
  ...
  const workspaceRows = workspaces.map((workspace) => (
    <WorkspaceAccessRow
      ...
      libraries={librariesByWorkspace.get(workspace.id) ?? []}
  ...
  if (isLoading) {
    ...
    accessContent = <WorkbenchEmptyState title={t('admin.users.access.empty')} />
  } else {
    accessContent = (
      <ScrollArea className="max-h-[55vh] pr-3">
        <div className="space-y-1.5">{workspaceRows}</div>
      </ScrollArea>
    )
  }
