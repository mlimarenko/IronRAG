...
function visiblePermGroups(scope: TokenScope): { group: string; perms: PermMeta[] }[] {
  ...
  const filtered = PERM_META.filter((p) => scope === 'system' || scope === 'workspace' || !p.wsOnly)
...
function groupTokenPermissions(token: APIToken): { group: string; permissions: string[] }[] {
  const grouped = token.grants.reduce<Record<string, Set<string>>>((acc, grant) => {
    ...
    const permissions = acc[group] ?? new Set<string>()
...
function removePermission(
  permissions: IamPermissionKind[],
  permissionToRemove: IamPermissionKind,
): IamPermissionKind[] {
  return permissions.filter((permission) => permission !== permissionToRemove)
}
...
function updateSelectedValue(
  ...
  if (!isChecked) return selectedValues.filter((selectedValue) => selectedValue !== value)
  if (selectedValues.includes(value)) return selectedValues
  return [...selectedValues, value]
}

type PermissionGroupGridProps = Readonly<{
  t: TFunction
  tokenScope: TokenScope
  selectedPermissions: IamPermissionKind[]
  onPermissionsChange: (permissions: IamPermissionKind[]) => void
}>

function PermissionGroupGrid({
  t,
  tokenScope,
  selectedPermissions,
  onPermissionsChange,
    ...
    <div className="mt-2 grid grid-cols-2 gap-2">
      {groups.map(({ group, perms }) => (
        <div key={group} className="rounded-xl p-2.5 bg-surface-sunken space-y-1">
          <div className="section-label mb-1">{permissionGroupLabel(group, t)}</div>
          {perms.map((perm) => {
            ...
            return (
              <label
                ...
                className={`flex items-center gap-2 rounded-lg px-2 py-1 text-xs transition-colors ${
                  isImplied ? 'opacity-50 cursor-default' : 'cursor-pointer hover:bg-accent/40'
                }`}
              >
                <Checkbox
                  id={`perm-${perm.key}`}
                  ...
                  onCheckedChange={(checked) => {
                    let nextPermissions: IamPermissionKind[]
                    if (!checked) {
                      nextPermissions = removePermission(selectedPermissions, perm.key)
                    } else if (selectedPermissions.includes(perm.key)) {
                      nextPermissions = selectedPermissions
                    } else {
                      nextPermissions = [...selectedPermissions, perm.key]
                    }
...
function resolveTokenWorkspaceId(
  workspaceId: string,
  defaultWorkspaceId: string,
  workspaces: CatalogWorkspaceResponse[],
): string {
  if (workspaces.some((workspace) => workspace.id === workspaceId)) return workspaceId
...
function canCreateToken({
  ...
  if (!formIsValid || isMintPending) return false
  if (scope === 'system') return true
  if (!workspaceId || isWorkspacesLoading || workspacesError) return false
  if (scope !== 'library') return true
  return !isLibrariesLoading && !librariesError && selectedLibraryCount > 0
}

function optionalQueryError(error: unknown, fallback: string): string | null {
...
function TokenSummary({
  ...
  if (loadError) {
    return <span className="text-status-failed">{loadError}</span>
  }
  return (
    <>
...
export function AccessTab({ t, activeWorkspaceId, active }: AccessTabProps) {
    ...
    () =>
      z
        .object({
          ...
          permissionKinds: z
            .array(
              z.custom<IamPermissionKind>(
                (value) =>
                  typeof value === 'string' && PERM_META.some((perm) => perm.key === value),
  ...
  const { getValues: getTokenValues, reset: resetTokenForm, setValue: setTokenValue } = tokenForm
  ...
  const loadError = optionalQueryError(tokensQuery.error, t('admin.loadTokensFailed'))
  ...
  const tokenWorkspacesError = optionalQueryError(
    workspacesQuery.error,
    t('admin.loadWorkspacesFailed'),
  )
  ...
  const selectedTokenWorkspaceId = resolveTokenWorkspaceId(
    tokenWorkspaceId,
  ...
  const tokenLibrariesLoading =
    librariesQuery.isLoading && createOpen && Boolean(selectedTokenWorkspaceId)
  const tokenLibrariesError = optionalQueryError(
    librariesQuery.error,
    t('admin.loadLibrariesFailed'),
  )
  ...
  >({
    ...
    onMutate: async ({ optimisticToken }) => {
      ...
      const previousTokens = queryClient.getQueryData<TokenResponse[]>(tokenListQuery.queryKey)
      const previousSelectedTokenId = selectedTokenId
      queryClient.setQueryData<TokenResponse[]>(tokenListQuery.queryKey, (current = []) => [
        optimisticToken,
        ...current,
      ])
    ...
    onSuccess: (data, { optimisticToken }) => {
      queryClient.setQueryData<TokenResponse[]>(tokenListQuery.queryKey, (current = []) =>
        current.map((token) =>
          token.principalId === optimisticToken.principalId ? data.apiToken : token,
        ),
      )
    ...
    onSettled: async () => {
      await queryClient.invalidateQueries({ queryKey: tokenListQuery.queryKey })
    },
  })

  const revokeTokenMutation = useMutation<void, unknown, APIToken, TokenMutationContext>({
  ...
  const deleteTokenMutation = useMutation<void, unknown, APIToken, TokenMutationContext>({
    ...
    onMutate: async (token) => {
      ...
      setSelectedTokenId(
        previousSelectedTokenId === token.id
          ? (previousTokens?.find((candidate) => candidate.principalId !== token.id)?.principalId ??
              null)
  ...
  const canCreate = canCreateToken({
    formIsValid: tokenForm.formState.isValid,
    scope: tokenScope,
    workspaceId: selectedTokenWorkspaceId,
    isWorkspacesLoading: tokenWorkspacesLoading,
    workspacesError: tokenWorkspacesError,
    isLibrariesLoading: tokenLibrariesLoading,
    librariesError: tokenLibrariesError,
    selectedLibraryCount: selectedActiveLibraryIds.length,
    isMintPending: mintTokenMutation.isPending,
  })
      ...
      <div className="flex flex-col">
        <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex gap-4 text-xs font-semibold">
            <TokenSummary t={t} isLoading={loading} loadError={loadError} tokens={tokens} />
          </div>
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="mt-3 flex flex-wrap gap-1.5">
                        {uniquePermissionLabels(token, t)
                          .slice(0, 3)
                          .map((label) => (
                  ...
                  return (
                    <tr
                      ...
                      className={`cursor-pointer border-b border-border/50 transition-colors ${
                        selected ? 'border-l-2 border-l-primary bg-primary/5' : 'hover:bg-accent/30'
                      }`}
                      onClick={() => setSelectedTokenId(token.id)}
                    >
              ...
              <div className="workbench-surface space-y-4 p-5">
                ...
                <div className="rounded-xl bg-surface-sunken/70 p-4">
                  <div className="space-y-4">
                    <div>
                      <div className="section-label">{t('admin.prefix')}</div>
                      <div className="mt-1 break-all font-mono text-xs font-bold">
                        {selectedToken.tokenPrefix}...
                      </div>
                    </div>
                    <div>
                      <div className="section-label">{t('admin.issuedBy')}</div>
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                      <div>
                        <div className="section-label">{t('admin.expires')}</div>
                      ...
                      <div>
                        <div className="section-label">{t('admin.lastUsed')}</div>
                        <div className="mt-1 text-sm font-medium">
                ...
                <div className="rounded-xl bg-surface-sunken/70 p-4">
                  <div className="section-label">{t('admin.scope')}</div>
                  <div className="mt-1 text-sm font-semibold">
                    {tokenScopeHeading(selectedToken, t)}
                  </div>
                  {selectedToken.scope.workspace ? (
                    <div className="mt-3">
                      <div className="section-label">{t('admin.tokenWorkspace')}</div>
                      <div className="mt-1 break-words text-sm font-medium">
                  ...
                  {selectedToken.scope.libraries.length > 0 ? (
                    <div className="mt-3">
                      <div className="section-label">{t('admin.tokenLibraries')}</div>
                ...
                <div className="rounded-xl bg-surface-sunken/70 p-4">
                  <div className="section-label">{t('admin.tokenPermissions')}</div>
                  {selectedTokenPermissionGroups.length === 0 ? (
                    <p className="mt-2 text-sm text-muted-foreground">
                      {t('admin.tokenNoPermissions')}
                    </p>
                  ) : (
                    <div className="mt-3 space-y-3">
                      {selectedTokenPermissionGroups.map(({ group, permissions }) => (
                        <div key={group}>
                          <div className="section-label">{permissionGroupLabel(group, t)}</div>
                          <div className="mt-2 flex flex-wrap gap-2">
            ...
            <FormSelectField
              ...
              onValueChange={(value) => {
                ...
                const allowed = new Set(
                  PERM_META.filter(
                    (permission) =>
                      nextScope === 'system' || nextScope === 'workspace' || !permission.wsOnly,
            ...
            {tokenScope === 'library' && (
              <div>
                ...
                <div className="mt-2 space-y-1.5 max-h-40 overflow-y-auto p-3 rounded-xl bg-surface-sunken">
                  <DataState
                    query={{
                      isLoading: tokenLibrariesLoading,
                      error: tokenLibrariesError,
                      data: tokenLibraries,
                    }}
                    emptyRender={
                      ...
                      libraries.map((library) => (
                        <div key={library.id} className="flex items-center gap-2.5">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) => {
                              const nextLibraryIds = updateSelectedValue(
                                selectedLibraryIds,
                                library.id,
                                checked,
                              )
