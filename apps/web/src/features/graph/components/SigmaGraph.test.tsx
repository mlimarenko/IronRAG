...
const { sigmaInstances } = vi.hoisted(() => ({
  sigmaInstances: [] as Array<{
    ...
    getCamera: () => { animatedReset: ReturnType<typeof vi.fn> }
...
vi.mock('graphology', () => {
  class FakeGraph {
    ...
    updateEachNodeAttributes(
      callback: (id: string, attributes: Record<string, unknown>) => Record<string, unknown>,
    ) {
...
describe('SigmaGraph', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(computeGraphLayoutOffThread).mockImplementation(async ({ nodes }) => ({
      type: 'result',
      requestId: 1,
  ...
  it('reports camera reset failures without surfacing an unhandled rejection', async () => {
    const resetFailure = new Error('camera reset failed')
    const reportError = vi.spyOn(console, 'error').mockImplementation(() => undefined)

    const onFitViewReady = vi.fn()
    ...
    const fitView = onFitViewReady.mock.calls[0]?.[0]
    if (!sigma || !fitView) throw new Error('sigma fit view not initialized')
    sigma.getCamera().animatedReset.mockRejectedValue(resetFailure)
    ...
    expect(reportError).toHaveBeenCalledWith('[graph] camera reset failed', resetFailure)
  })
  ...
  it('updates the indexed all-edge texture without rebuilding the edge buffer on dense layout switch', async () => {
    ...
    expect(mockedComputeLayout).toHaveBeenLastCalledWith(
      expect.objectContaining({ layout: 'components' }),
    )
    ...
    expect(
      nodeRefreshCalls.every((options) => (options.partialGraph?.nodes?.length ?? 0) <= 5000),
    ).toBe(true)
    expect(
      sigma.refresh.mock.calls.some(([options]) => {
        const refreshOptions = options as {
          partialGraph?: unknown
          schedule?: boolean
          skipIndexation?: boolean
        }
        return (
          refreshOptions.schedule === true &&
          refreshOptions.skipIndexation === true &&
          !refreshOptions.partialGraph
        )
      }),
    ).toBe(false)
    expect(allEdgesCanvas?.getAttribute('data-all-edges-layer')).toBe('indexed')
  ...
  it('refreshes sampled Sigma edges on dense layout switch when the indexed all-edge overlay is inactive', async () => {
    ...
    expect(
      edgeRefreshCalls.every((options) => (options.partialGraph?.edges?.length ?? 0) <= 4000),
    ).toBe(true)
  })
  ...
  it('keeps the indexed all-edge overlay hidden and undrawn while a dense layout switch is suspended', async () => {
    ...
    const sigma = sigmaInstances[0]
    if (!sigma) throw new Error('sigma instance not created')
    const allEdgesCanvas = container.querySelector<HTMLCanvasElement>(
      'canvas[data-all-edges-layer="indexed"]',
    )
    expect(allEdgesCanvas).not.toBeNull()
