...
import type { DocumentsStatusBucket, UpdateSearchParamState } from './documentsPageState'

type DocumentsFiltersBarProps = Readonly<{
  libraryCost: number
    ...
    <div className="px-6 py-3 border-b flex flex-wrap items-center gap-3 bg-surface-sunken/50">
          ...
          return (
            ...
            >
              ...
              {active && bucket.count != null && bucket.count > 0 && (
                <span className="tabular-nums text-2xs opacity-70">{bucket.count}</span>
              )}
            </button>
          )
        })}
      ...
      <div className="ml-auto flex items-center gap-3">
        <Tooltip>
          ...
          <TooltipContent className="max-w-64">{t('documents.detailColumnsHint')}</TooltipContent>
        </Tooltip>
        <Button
          size="sm"
        ...
        {showCostSummary && (
          <div className="hidden flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground lg:flex">
            <span>
              ...
              <span className="font-bold tabular-nums">${libraryCost.toFixed(3)}</span>
            </span>
            <span>
              {t('documents.workspaceCost')}:{' '}
              <span className="font-bold tabular-nums">${workspaceCost.toFixed(3)}</span>
            </span>
          </div>
        )}
      </div>
