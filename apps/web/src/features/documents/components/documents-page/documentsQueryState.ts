...
export function useCursorStack({
  ...
  const cursorScopeKey = useMemo(
    () => JSON.stringify([activeLibraryId, debouncedSearch, sortValue, statusBucket, pageSize]),
  ...
  const cursorStack = cursorStackState.scopeKey === cursorScopeKey ? cursorStackState.stack : [null]
  const setCursorStack = useCallback(
    (action: SetStateAction<(string | null)[]>) => {
      setCursorStackState((prev) => {
        const currentStack = prev.scopeKey === cursorScopeKey ? prev.stack : [null]
        const stack = typeof action === 'function' ? action(currentStack) : action
...
export function useListPollGrace() {
  ...
  useEffect(() => {
    ...
    const timeoutMs = Math.min(LIST_POLL_INTERVAL_MS, listPollGraceUntil - listPollClockMs)
    const timeoutId = window.setTimeout(() => setListPollClockMs(Date.now()), timeoutMs)
...
export function buildPaginationState({
  ...
  const visibleRangeStart = itemCount === 0 ? 0 : (currentPageNumber - 1) * pageSize + 1
  const visibleRangeEnd = itemCount === 0 ? 0 : (currentPageNumber - 1) * pageSize + itemCount
  return {
    ...
    pageSize,
    show: itemCount > 0 || cursorStack.length > 1 || nextCursor != null || (filteredTotal ?? 0) > 0,
    totalPages:
