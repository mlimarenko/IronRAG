...
describe('useWebIngestController optimistic policy save', () => {
  ...
  it('shows the saved policy optimistically and rolls back with a toast on failure', async () => {
    ...
    expect(container.querySelector('[data-testid="saved-count"]')).toHaveTextContent('0')
    expect(toastErrorMock).toHaveBeenCalledWith(expect.stringContaining('policy unavailable'))
  })
  ...
  it('blocks subdomain boundary submissions for IP seed hosts', async () => {
    ...
    expect(toastErrorMock).toHaveBeenCalledWith('documents.subdomainBoundaryRequiresDnsHost')
  })
})
