...
const { editorState, editorChain, mockEditor, useEditorMock, getLatestEditorConfig } = vi.hoisted(
  () => {
    const editorState = {
      markdown: '',
      activeTable: false,
...
describe('DocumentEditorShell', () => {
  ...
  it('shows code-focused copy for code documents', async () => {
    ...
    expect(container.textContent).toContain(
      'Code files keep a monospace workspace with scrollable long lines.',
    )
  ...
  it('keeps line wrapping enabled by default and lets the toolbar disable it', async () => {
    ...
    expect(container.querySelector('textarea')?.className).not.toContain(
      'document-editor-raw-textarea--wrap',
    )
  })
  ...
  it('uses viewport-width table content while line wrapping is enabled', async () => {
    ...
    expect(
      container.querySelector('[data-testid="document-editor-table-content"]')?.className,
    ).toContain('w-max')
  })
