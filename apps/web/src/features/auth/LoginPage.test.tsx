...
const bootstrapBindingPurposes: AiBindingPurpose[] = [
  'extract_text',
  'extract_graph',
  'embed_chunk',
  'query_compile',
...
function bootstrapBinding(bindingPurpose: AiBindingPurpose, index: number) {
  return {
    bindingPurpose,
...
describe('LoginPage operator-safe errors', () => {
  ...
  function completeSetupButton() {
    const button = Array.from(container.querySelectorAll('button')).find((candidate) =>
      candidate.textContent?.includes('Complete Setup'),
    )
    if (!button) {
  ...
  it('renders only canonical bootstrap binding purposes', async () => {
    mocks.app.isBootstrapRequired = true
    mocks.bootstrapQueryFn.mockResolvedValue({
    ...
    await waitFor(() => {
      ...
      expect(container.textContent).toContain('AI Assistant (MCP Agent)')
    })

    expect(container.textContent).toContain(
      'Use one multimodal model for OCR, images, and complex document content',
    )
