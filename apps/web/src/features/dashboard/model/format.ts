...
import type { DashboardAttentionItem, GraphStatus, MessageLevel, WebIngestRunState } from './types'
...
export function toneClass(tone: ToneKey): { containerClass: string; iconClass: string } {
  switch (tone) {
    case 'ready':
      return {
        containerClass: 'bg-status-ready-bg ring-1 ring-status-ready-ring/35',
        iconClass: 'text-status-ready',
      }
    case 'warning':
      return {
        containerClass: 'bg-status-warning-bg ring-1 ring-status-warning-ring/35',
        iconClass: 'text-status-warning',
      }
    case 'failed':
      return {
        containerClass: 'bg-status-failed-bg ring-1 ring-status-failed-ring/35',
        iconClass: 'text-status-failed',
      }
    case 'processing':
      return {
        containerClass: 'bg-status-processing-bg ring-1 ring-status-processing-ring/35',
