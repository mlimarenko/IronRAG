...
function SummaryCardsImpl({ cards, onNavigate }: Readonly<SummaryCardsProps>) {
  return (
    <div className="grid gap-2 sm:grid-cols-3">
      {cards.map((card) => {
        ...
        return (
          ...
          >
            <div
              className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-md ${tone.containerClass}`}
            >
              <Icon className={`h-3.5 w-3.5 ${tone.iconClass}`} />
            </div>
            <div className="min-w-0 flex-1">
              <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                <div className="text-base font-semibold tabular-nums sm:text-lg">{card.value}</div>
