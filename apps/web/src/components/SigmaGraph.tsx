  ...
  useEffect(() => {
    ...
    graph.forEachNode((node) => {
      ...
      if (selectedId) {
        ...
        graph.setNodeAttribute(node, 'color', isSelected || isNeighbor ? baseColor : baseColor + '30');
    ...
    graph.forEachEdge((edge) => {
      ...
      if (selectedId && (source === selectedId || target === selectedId)) {
        graph.setEdgeAttribute(edge, 'color', 'rgba(59, 130, 246, 0.7)');
        graph.setEdgeAttribute(edge, 'size', 1.5);
      } else {
        graph.setEdgeAttribute(edge, 'color', selectedId ? 'rgba(148, 163, 184, 0.05)' : 'rgba(148, 163, 184, 0.15)');
