...
describe('ProviderCredentialFields', () => {
  ...
  it('renders a fixed endpoint as readable read-only text instead of a disabled input', async () => {
    ...
    expect(container.querySelector('#provider-base-url')?.textContent).toBe(
      'https://provider.example/v1',
    )
    expect(container.querySelector('#provider-base-url')?.getAttribute('aria-readonly')).toBeNull()
    expect(container.querySelector('#provider-base-url')?.getAttribute('role')).toBeNull()
    expect(container.querySelector('#provider-base-url')?.getAttribute('tabindex')).toBeNull()
