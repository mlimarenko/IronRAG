...
type ProviderCredentialFieldsProps = Readonly<{
...
export function ProviderCredentialFields({
  ...
  const apiKeyDescriptionIds =
    [apiKeyError ? apiKeyErrorId : '', apiKeyHint ? apiKeyHintId : ''].filter(Boolean).join(' ') ||
    undefined
  ...
  return (
    <div className="space-y-4">
      {baseUrlVisible && (
        <div>
          ...
          {baseUrlEditable ? (
            ...
            <output
              id={baseUrlFieldId}
              className="mt-2 block select-text rounded-lg border border-border bg-surface-sunken px-3 py-2 font-mono text-xs leading-relaxed text-foreground [overflow-wrap:anywhere]"
              aria-describedby={baseUrlDescriptionIds}
      ...
      <div>
        ...
        <Input
          ...
          placeholder={
            preserveExistingSecret
              ? (labels.keepSecretPlaceholder ?? labels.apiKeyPlaceholder)
          ...
          aria-required={
            (credentialPolicy?.apiKeyRequired && !apiKeyDisabled && !preserveExistingSecret) ||
            undefined
          }
        />
        {apiKeyError && (
        ...
        {apiKeyHint && (
          <p id={apiKeyHintId} className="mt-2 text-xs text-muted-foreground">
            {apiKeyHint}
          </p>
        )}
      </div>
