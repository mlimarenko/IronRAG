  ...
  return (
    <div className="rounded-xl border border-border/60 bg-surface-sunken p-4 space-y-3">
      <div className="flex items-start justify-between gap-3">
        ...
        >
          <Icon className={ready ? 'h-3.5 w-3.5 text-status-ready' : 'h-3.5 w-3.5 text-status-warning'} />
          <span>{ready ? readyLabel : attentionLabel}</span>
        </div>
      </div>
      <div className="grid gap-2 text-xs sm:grid-cols-2">
        {rows.map(row => (
          <div key={row.label} className="min-w-0 rounded-lg border border-border/50 bg-background/70 px-3 py-2">
            <div className="text-muted-foreground">{row.label}</div>
            <div className="mt-1 font-medium text-foreground [overflow-wrap:anywhere]">{row.value}</div>
          </div>
        ))}
      </div>
    </div>
