...
function readLocalStorageState<T>({ key, defaultValue, parse }: UseLocalStorageStateOptions<T>): T {
...
export function useLocalStorageState<T>(
  ...
  const [state, setState] = useState<T>(() => readLocalStorageState({ key, defaultValue, parse }))
