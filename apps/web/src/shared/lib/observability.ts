...
const hasZone = () =>
  typeof (globalThis as typeof globalThis & { Zone?: unknown }).Zone !==
  "undefined";
...
async function startObservability(): Promise<void> {
  ...
  const serviceVersion =
    readEnv("VITE_OTEL_SERVICE_VERSION") ??
    readEnv("VITE_APP_VERSION") ??
    "0.0.0";
  const deploymentEnvironment =
    readEnv("VITE_OTEL_DEPLOYMENT_ENVIRONMENT") ??
  ...
  const resource = resources.resourceFromAttributes({
    ...
    [incubatingSemanticConventions.ATTR_DEPLOYMENT_ENVIRONMENT]:
  ...
  const provider = new traceWeb.WebTracerProvider({
    resource,
    spanProcessors: [
      new traceWeb.BatchSpanProcessor(
        new exporter.OTLPTraceExporter({ url: endpoint }),
      ),
    ],
  });
