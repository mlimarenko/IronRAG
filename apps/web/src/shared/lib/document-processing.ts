...
export function buildUploadFailureNotice(
  ...
  const rejectionCause =
    stringRecordValue(details, 'rejectionCause') ?? stringRecordValue(details, 'rejection_cause')
  const operatorAction =
    stringRecordValue(details, 'operatorAction') ?? stringRecordValue(details, 'operator_action')
  const apiMessage =
  ...
  const action =
    ...
    fallback
  const diagnosticMessage =
    [apiMessage, rejectionCause, operatorAction]
