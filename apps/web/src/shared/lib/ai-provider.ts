...
function assertRecord(
  ...
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new TypeError(`Invalid AI provider metadata: ${policyName} must be an object`)
  }
}
...
function assertBooleanField(
  ...
  if (typeof value[fieldName] !== 'boolean') {
    throw new TypeError(`Invalid AI provider metadata: ${policyName}.${fieldName} must be boolean`)
  }
}
...
function assertStringField(
  ...
  if (typeof value[fieldName] !== 'string' || value[fieldName].trim() === '') {
    throw new TypeError(`Invalid AI provider metadata: ${policyName}.${fieldName} must be string`)
  }
}
...
function assertStringEnumField(
  ...
  if (typeof value[fieldName] !== 'string' || !allowedValues.has(value[fieldName])) {
    throw new TypeError(`Invalid AI provider metadata: ${policyName}.${fieldName} is not canonical`)
  }
}
...
function assertBaseUrlPolicy(value: unknown): asserts value is AIProviderBaseUrlPolicy {
  ...
  if (
    !Array.isArray(value.trimSuffixes) ||
    value.trimSuffixes.some((entry) => typeof entry !== 'string')
  ) {
    throw new TypeError('Invalid AI provider metadata: baseUrlPolicy.trimSuffixes must be string[]')
  }
}
...
function assertModelDiscovery(value: unknown): asserts value is AIProviderModelDiscovery {
  ...
  if (!Array.isArray(value.paths)) {
    throw new TypeError('Invalid AI provider metadata: modelDiscovery.paths must be an array')
  }
...
export function normalizeProviderBaseUrl(
  ...
  return trimSuffixes.reduce((normalized, suffix) => {
    const escapedSuffix = suffix.replace(/[.*+?^${}()|[\]\\]/gu, String.raw`\$&`)
    return normalized.replace(new RegExp(`${escapedSuffix}/?$`, 'u'), '')
  }, baseUrl)
}

export function shouldRenderBaseUrlInput(
  provider: (CredentialPolicyInput & Pick<BaseUrlPolicyInput, 'defaultBaseUrl'>) | null | undefined,
): boolean {
  if (!provider) {
    return false
  }
...
export function canEditProviderBaseUrl(
  provider: Pick<BaseUrlPolicyInput, 'credentialSource' | 'baseUrlPolicy'> | null | undefined,
): boolean {
...
export function shouldRefreshCredentialModels(
  provider: Pick<AIProvider, 'modelDiscovery'> | null | undefined,
): boolean {
  if (!provider) {
    return false
  }
