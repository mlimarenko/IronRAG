...
type SubmitWithMutationOptions<TValues extends FieldValues, TResult = unknown> = {
  errorMessage?: string | ((error: unknown) => string)
  onError?: (error: unknown, values: TValues) => void
  onSuccess?: (result: TResult, values: TValues) => void
}

type UseTypedFormOptions<TSchema extends z.ZodType> = Omit<
  UseFormProps<FormInput<TSchema>, unknown, FormValues<TSchema>>,
  'resolver'
> & {
...
export type TypedFormReturn<TSchema extends z.ZodType> = UseFormReturn<
  FormInput<TSchema>,
  unknown,
  FormValues<TSchema>
> & {
...
export function useTypedForm<TSchema extends z.ZodType>({
  ...
  const form = useForm<FormInput<TSchema>, unknown, FormValues<TSchema>>({
    ...options,
    resolver: zodResolver(schema as z.ZodType<FormValues<TSchema>, FormInput<TSchema>>),
  })
