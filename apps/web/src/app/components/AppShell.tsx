...
export function AppShell({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  const [sidebarCollapsed, setSidebarCollapsed] = useState(() => {
    try {
      return localStorage.getItem('ironrag.sidebarCollapsed') === '1'
    } catch {
      return false
    }
  })
  const toggleSidebar = () =>
    setSidebarCollapsed((prev) => {
      const next = !prev
      try {
        localStorage.setItem('ironrag.sidebarCollapsed', next ? '1' : '0')
      } catch {
        /* ignore */
      }
      return next
  ...
  const filteredLibraries = useMemo(
    () =>
      librarySearchValue
        ? libraries.filter((library) => library.name.toLowerCase().includes(librarySearchValue))
        : libraries,
  ...
  const handleDeleteWorkspace = async () => {
    if (deleteConfirmName !== activeWorkspace?.name || deleteSubmitting) return
    ...
    try {
      ...
      pollCatalogDeletion(
        ...
        toast.error(errorMessage(error, t('shell.workspaceDeleteFailed')), { id: toastId })
      })
    } catch (err: unknown) {
  ...
  const handleDeleteLibrary = async () => {
    if (deleteConfirmName !== activeLibrary?.name || activeWorkspace === null || deleteSubmitting) {
      return
    }
    const workspace = activeWorkspace
    ...
    try {
      ...
      pollCatalogDeletion(
        ...
        toast.error(errorMessage(error, t('shell.libraryDeleteFailed')), { id: toastId })
      })
    } catch (err: unknown) {
    ...
    return (
      ...
      >
        ...
        <span className="flex min-w-0 flex-1 flex-col leading-tight">
          <span className="section-label text-shell-muted">{caption}</span>
          <span
  ...
  const aiWarningButton = (fullWidth = false, collapsed = false) => (
    <button
      type="button"
      onClick={() => {
        navigate('/admin/ai').catch(() => undefined)
  ...
  const sidebarLinkClass = (active: boolean, nested = false, collapsed = false) => {
    const spacingClass = collapsed ? 'justify-center px-0' : 'px-3'
    let nestedClass = ''
    if (nested) nestedClass = collapsed ? 'text-xs' : 'pl-9 text-xs'
    ...
    return `flex min-w-0 items-center gap-2 rounded-lg py-2 text-sm font-semibold transition-colors ${spacingClass} ${nestedClass} ${stateClass}`
  }
      ...
      >
        ...
        {canAccessAdmin && (
          <div className="space-y-3 border-t border-shell-border pt-4">
            {!collapsed && (
              <div className="px-3 section-label text-shell-muted">{t('nav.admin')}</div>
            )}
                  ...
                  {sections.map((section) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {!collapsed && (
                          <span className="truncate">{t(`admin.nav.${section.labelKey}`)}</span>
                        )}
                      </Link>
                    )
                  })}
        ...
        {mobileMenuOpen && (
          <>
            <button
              ...
              onClick={() => setMobileMenuOpen(false)}
            />
            <aside className="fixed inset-y-0 left-0 z-50 flex w-[min(20rem,88vw)] border-r border-shell-border bg-shell shadow-overlay md:hidden">
              <button
                type="button"
      ...
      <Dialog open={createLibOpen} onOpenChange={setCreateLibOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {t('shell.createLibraryDesc', { name: activeWorkspace?.name })}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div>
              ...
              <Input
                ...
                placeholder={t('shell.libraryNamePlaceholder')}
                className="mt-1.5"
              />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setCreateLibOpen(false)}>
              {t('shell.cancel')}
            </Button>
            <Button onClick={handleCreateLibrary} disabled={!newLibName.trim()}>
              {t('shell.create')}
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={deleteWsOpen} onOpenChange={setDeleteWsOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {t('shell.deleteWorkspaceDesc', { name: activeWorkspace?.name })}
            </DialogDescription>
          </DialogHeader>
          <div>
            <Label htmlFor="del-ws-confirm">
              {t('shell.typeToConfirm', { name: activeWorkspace?.name })}
            </Label>
            <Input
              id="del-ws-confirm"
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={deleteConfirmName !== activeWorkspace?.name || deleteSubmitting}
            >
              {t('shell.delete')}
            </Button>
          </DialogFooter>
      ...
      <Dialog open={deleteLibOpen} onOpenChange={setDeleteLibOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {t('shell.deleteLibraryDesc', { name: activeLibrary?.name })}
            </DialogDescription>
          </DialogHeader>
          <div>
            <Label htmlFor="del-lib-confirm">
              {t('shell.typeToConfirm', { name: activeLibrary?.name })}
            </Label>
            <Input
              id="del-lib-confirm"
              value={deleteConfirmName}
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setDeleteLibOpen(false)}
              disabled={deleteSubmitting}
            >
              {t('shell.cancel')}
            </Button>
            <Button
              ...
              disabled={deleteConfirmName !== activeLibrary?.name || deleteSubmitting}
            >
              {t('shell.delete')}
            </Button>
          </DialogFooter>
