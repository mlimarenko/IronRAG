...
pub(crate) fn descriptor(name: &str) -> Option<McpToolDescriptor> {
    match name {
        SEARCH_DOCUMENTS_TOOL_NAME => Some(McpToolDescriptor {
            ...
            description: "Search authorized library memory and return document-level candidates. Use this when you need to choose or inspect source documents directly; for ordinary content answers, setup/configuration questions, troubleshooting, release summaries, or inventories of values inside documents, `grounded_answer` is usually the better first probe. The search response alone is NOT enough to ground an answer: it tells you where to look, not what the source says. Follow relevant hits with `read_document` before using them as evidence. By default this tool returns only documents whose content is readable enough to inspect; unreadable/failed documents are hidden unless you explicitly opt in for diagnostics. Each hit carries `suggestedStartOffset` — the character offset for a first read window that should contain the best-matching chunk inside the full document; ALWAYS pass this value as `startOffset` to `read_document` on the first call for that document so your first read window lands on the relevant paragraph instead of the PDF's table of contents. Rules: (1) NEVER rerun this tool with the same `query`+`libraries`+`limit` payload in one turn; if the first call returned nothing useful, narrow or broaden the query. (2) If the top hit is clearly the right document, prefer one `read_document` call with `startOffset=suggestedStartOffset` over issuing more searches. (3) Keep `limit` small (3-10). (4) Use `includeUnreadable=true` only for diagnostics or ops investigation, not for normal user-facing answers.",
            input_schema: json!({
                "type": "object",
                "required": ["query"],
                "properties": {
        ...
        READ_DOCUMENT_TOOL_NAME => Some(McpToolDescriptor {
            ...
            description: "Read one document's text content. Use this after `search_documents`, after a `grounded_answer` result points to a source that needs verification or expansion, or when you already know the `documentId`. `mode=full` returns the whole document when it fits in one read window; otherwise it returns one paged window and includes `hasMore` plus a `continuationToken`. Rules: (1) On the first read after `search_documents`, pass `startOffset` equal to the hit's `suggestedStartOffset` when present. For a small document in `mode=full`, the backend still returns the whole document from offset 0 so the matched tail cannot hide earlier evidence. (2) If `hasMore` is true and the answer likely depends on later text, call this tool again with the same `documentId` and returned `continuationToken`; do not switch away just because the first window is thin. (3) NEVER rerun with an identical payload — always advance the token. (4) A PDF's first windows can be table of contents or section headers. If the only thing you see is ToC, you have NOT read the content yet. (5) For setup/configuration answers, keep reading the focused source until you have the nearby package/module, path, and parameter/default/example fragments, or can state which of those are absent. (6) For image-backed documents the response can include `sourceAccess` and a `visualDescription` derived from the original source image; prefer that grounded description over guessing from OCR fragments.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "documentId": {
