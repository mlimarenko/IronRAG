...
//!
//! Phase 1 scope:
//!   - input: `libraryId`, `query`, optional `topK`, optional
//!     `includeDebug`
//!   - output: grounded answer text, citation list, verifier verdict,
//!     `runtimeExecutionId`, `conversationId`, `executionId`
//! Multi-turn history is intentionally out-of-scope for phase 1 —
//! external agents pass their own rolling conversation context to the
//! LLM side and call `grounded_answer` statelessly per question.

use serde_json::{Value, json};
use uuid::Uuid;

use crate::{
    domains::query::QueryVerificationState,
    interfaces::http::{
        authorization::{POLICY_QUERY_RUN, load_library_and_authorize},
        ...
        input_schema: json!({
            ...
            "properties": {
                "libraryId": {
                    "type": "string",
                    "format": "uuid",
                    "description": "Target library UUID. The token MUST have query_run on this library."
                },
                "query": {
                    "type": "string",
...
async fn grounded_answer(context: ToolCallContext<'_>, arguments: &Value) -> McpToolResult {
    ...
    let library = match load_library_and_authorize(
        context.auth,
        context.state,
        parsed.library_id,
        POLICY_QUERY_RUN,
