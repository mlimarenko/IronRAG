...
use super::super::{
    ...
    audit::{build_mcp_web_ingest_subjects, record_canonical_mcp_audit},
...
pub(crate) fn descriptor(name: &str) -> Option<McpToolDescriptor> {
    match name {
        "submit_web_run" => Some(McpToolDescriptor {
            name: "submit_web_run",
        ...
        "get_web_run" => Some(McpToolDescriptor {
            name: "get_web_run",
            ...
            input_schema: json!({
                ...
                "properties": {
                    "runId": {
                        "type": "string",
                        "format": "uuid",
                        "description": "Run UUID returned by submit_web_run."
                    }
                }
            }),
        }),
        "list_web_run_pages" => Some(McpToolDescriptor {
        ...
        "cancel_web_run" => Some(McpToolDescriptor {
            name: "cancel_web_run",
...
pub(crate) async fn call_tool(
    ...
    let result = match name {
        "submit_web_run" => submit_web_run(context, arguments).await,
        "get_web_run" => get_web_run(context, arguments).await,
        "list_web_run_pages" => list_web_run_pages(context, arguments).await,
        "cancel_web_run" => cancel_web_run(context, arguments).await,
        _ => return None,
    };
    Some(result)
}

async fn submit_web_run(context: ToolCallContext<'_>, arguments: &Value) -> McpToolResult {
...
async fn get_web_run(context: ToolCallContext<'_>, arguments: &Value) -> McpToolResult {
    match parse_tool_args::<McpGetWebIngestRunRequest>(arguments.clone()) {
        ...
        .await
        {
            Ok(payload) => ok_tool_result("Web ingest run loaded.", json!(payload)),
            Err(error) => tool_error_result(error),
        },
        Err(error) => tool_error_result(error),
    }
...
async fn list_web_run_pages(context: ToolCallContext<'_>, arguments: &Value) -> McpToolResult {
...
async fn cancel_web_run(context: ToolCallContext<'_>, arguments: &Value) -> McpToolResult {
...
mod tests {
    ...
    #[test]
    fn submit_web_run_boundary_policy_schema_matches_runtime_vocabulary() {
        let submit_tool = descriptor("submit_web_run").expect("submit tool descriptor");
