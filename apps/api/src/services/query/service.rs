...
use crate::{
    ...
    services::{
        content::source_access::describe_content_source,
...
const MAX_ANSWER_SOURCE_LINKS: usize = 5;
...
struct PreparedSegmentRevisionInfo {
    document_title: Option<String>,
    source_uri: Option<String>,
    source_access: Option<crate::domains::content::ContentSourceAccess>,
}

#[derive(Debug, Clone, Default)]
struct ExecutionPreparedReferenceContext {
    bundle_refs: Option<KnowledgeContextBundleReferenceSetRow>,
    fact_rank_refs: HashMap<Uuid, RankedBundleReference>,
    technical_fact_rows: Vec<KnowledgeTechnicalFactRow>,
    block_rank_refs: HashMap<Uuid, RankedBundleReference>,
    structured_block_rows: Vec<KnowledgeStructuredBlockRow>,
    segment_revision_info: HashMap<Uuid, PreparedSegmentRevisionInfo>,
}
...
async fn load_execution_prepared_reference_context(
    state: &AppState,
    execution_id: Uuid,
...
async fn load_prepared_segment_revision_info(
    ...
    state
        ...
        .into_iter()
        .map(|revision| {
            let source_descriptor = describe_content_source(
...
fn render_answer_source_links(references: &[PreparedSegmentReference]) -> Option<String> {
