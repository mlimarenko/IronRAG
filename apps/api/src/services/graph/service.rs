...
impl GraphService {
    ...
    pub async fn reconcile_revision_graph(
        ...
        // Serialize the merge phase per library to eliminate row-level lock
        // contention on `runtime_graph_evidence` between concurrent workers
        // processing different documents from the same library. The expensive
        // LLM-driven extraction phase has already completed in parallel before
        // this point, so the lock only covers the deterministic, fast graph
        // merge writes — and only across the same library, never globally.
