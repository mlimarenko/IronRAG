...
mod tests {
    ...
    fn normalize_strips_tracking_query_and_fragment() {
        let normalized =
            normalize_absolute_url("https://Docs.Example.com:443/path?q=1&utm_source=test#heading")
    ...
    fn resolve_relative_url_against_base() {
        let normalized =
            resolve_discovered_url("https://docs.example.com/guide/index.html", "../api")
    ...
    fn classify_same_host_vs_external() {
        assert_eq!(
            classify_host("https://docs.example.com/start", "https://docs.example.com/api")
                .expect("classify"),
            HostClassification::SameHost
        );
        assert_eq!(
            classify_host("https://docs.example.com/start", "https://example.org/api")
                .expect("classify"),
