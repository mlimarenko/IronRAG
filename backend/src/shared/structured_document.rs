...
pub fn validate_ordered_semantic_blocks(
    ...
    let known_ids = blocks.iter().map(|block| block.block_id).collect::<Vec<_>>();
    ...
    for block in blocks {
        ...
        if let Some(source_span) = &block.source_span
        ...
        if let Some(parent_block_id) = block.parent_block_id
            && !known_ids.contains(&parent_block_id)
        {
...
pub fn validate_structured_chunk_windows(
    ...
    let known_block_ids = blocks.iter().map(|block| block.block_id).collect::<Vec<_>>();
    let mut expected = 0_i32;
    ...
    fn validates_structured_revision_with_ordered_blocks_and_chunks() {
        ...
        let revision = StructuredDocumentRevisionData {
            ...
            chunk_windows: vec![StructuredChunkWindow {
                ...
                support_block_ids: vec![block.block_id],
