...
оперативные нюансы и пример пресета под 12 ГБ потребительский GPU.
...
## Пример пресета (12 ГБ VRAM, одна карта)
...
- Active `embed_chunk` / `query_retrieve` binding библиотеки определяет
  embedding dimension этой библиотеки.
...
frontend тоже нужно пересоздать (nginx upstream кэширует stale
backend IP).
...
|---|---|---|
| `ProviderUnavailable: failed to resolve chunk embedding dimensions for <uuid>` | Vector от embedding-модели не совпадает с active library binding или vector manifest lane | Проверь `embed_chunk` / `query_retrieve` bindings библиотеки и запусти `ironrag-maintenance rebuild vector-plane --source-library` для затронутой библиотеки. |
