...
|---|---|---|
| `binding_purpose` | enum `ai_binding_purpose` | один из 8 purpose'ов ниже |
...
| `query_retrieve` | query: эмбеддинг вопроса для vector search | эмбеддинги (не chat); обязана совпадать модель и размерность с `embed_chunk` для данной библиотеки |
...
- Ни та, ни другая не являются chat-моделью. Обе — embedding-модели:
  `embed_chunk` индексирует чанки при ingest, `query_retrieve` — делает
  эмбеддинг вопроса при query. Для данной библиотеки они должны использовать
  одну модель и одинаковую размерность — runtime проверяет это и падает
  явно при расхождении.
- Размерность — часть per-library контракта; vector material хранится в
  per-`(library, dim)` pgvector relations.
- Переход на модель с другой размерностью требует
  `ironrag-maintenance rebuild vector-plane --source-library` и переиндексации.
- Latency важна на ingest-масштабе для `embed_chunk`; для `query_retrieve`
  это критический путь запроса.
...
- **Embedding dimension drift.** Смена `embed_chunk`-модели между
  ...
  vector-migrate и переиндексации. Per-dim pgvector relations делают
  роутинг; old vector material остаётся пока не мигрирован.
