...
| [AI-BINDINGS.md](./AI-BINDINGS.md) | Модель AI bindings: 8 purpose'ов, scope-лестница, wire-level prompt layout, tradeoff'ы выбора модели, грабли prompt cache. |
...
vector rebuild от source-библиотеки с этим binding'ом. PostgreSQL хранит
...
- Смена embedding-модели с другой размерностью финализируется запуском
  vector rebuild, чтобы pgvector relations и сохранённые векторы менялись вместе.
