...
одна библиотека не будет перегружена. Дефолтные queue caps:
...
В Helm те же потолки задаются через `app.ingestionMaxParallelJobsGlobal`,
`app.ingestionMaxParallelJobsPerWorkspace` и
`app.ingestionMaxParallelJobsPerLibrary`.

Это deployment-wide потолки, а не гарантия, что один worker process сможет
вести 16 jobs одновременно. Каждый worker дополнительно применяет локальный
claim cap, рассчитанный из его cgroup soft memory limit, до того как забирает
новые canonical ingest jobs из Postgres. Это не даёт memory-heavy extraction
или graph-merge задачам накопиться в одном процессе на swapless host. Поднимать
ingest caps имеет смысл только вместе с host capacity, worker memory limit,
числом worker replicas, DB connection budget и provider concurrency budget.

Для 16 GiB host, где нужно реально заполнять дефолтные `16 / 8 / 4` queue caps,
лучше несколько умеренных воркеров, чем один очень большой. Практичная стартовая
точка:
...
С такими значениями у каждого worker достаточно memory и DB pool headroom,
чтобы lease-ить примерно четыре jobs, а очередь в БД всё равно ограничивает 16
глобально, 8 на workspace и 4 на library. Если видимый `leased` count
останавливается ниже 16, сначала проверьте, не лежат ли все доступные queued
jobs в одном workspace или library.
