...
| Frontend container build | blocked | `npm install` in container fails during `esbuild` postinstall with `spawn sh EACCES` | investigate container runtime / permissions workaround |
