...
<template>
  <SurfacePanel class="rr-feedback-card" :class="`rr-feedback-card--${kind}`">
    <div class="rr-feedback-card__copy">
      <h3>{{ title }}</h3>
      <p>{{ message }}</p>
      <ul v-if="details.length" class="rr-feedback-card__details">
        <li v-for="detail in details" :key="detail">
          {{ detail }}
        </li>
      </ul>
      <div v-if="actionLabel || $slots.action" class="rr-feedback-card__action">
        <slot name="action">
