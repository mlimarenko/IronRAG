...
embedding_model=${RUSTRAG_OPENAI_EMBEDDING_MODEL:-text-embedding-3-large}
answer_model=${RUSTRAG_OPENAI_ANSWER_MODEL:-gpt-5.4}
vision_model=${RUSTRAG_OPENAI_VISION_MODEL:-gpt-5-mini}
...
workspace_name="OpenAI Runtime Smoke $(date -u +%H%M%S)"
library_name="OpenAI Fixture Library"
...
smoke_create_api_token "${api_base}" "${cookie_jar}" "openai runtime smoke" "${full_scopes}" "${output_dir}/token.json"
...
smoke_validate_provider "${api_base}" "${token}" "${provider_kind}" "${answer_model}" chat "${output_dir}/validate-chat.json"
smoke_validate_provider "${api_base}" "${token}" "${provider_kind}" "${embedding_model}" embeddings "${output_dir}/validate-embeddings.json"
smoke_validate_provider "${api_base}" "${token}" "${provider_kind}" "${vision_model}" vision "${output_dir}/validate-vision.json"
...
jq -n \
  ...
  '{
    ...
    profile: {
      indexing: { providerKind: $provider, modelName: $indexing_model },
      embedding: { providerKind: $provider, modelName: $embedding_model },
      answer: { providerKind: $provider, modelName: $answer_model },
      vision: { providerKind: $provider, modelName: $vision_model }
    },
